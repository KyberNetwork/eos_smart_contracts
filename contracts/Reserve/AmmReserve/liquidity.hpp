...
double value_after_reducing_fixed_fee(struct liq_info &info, double val) {
    return (info.fixed_fee >= val) ? 0 : (val - info.fixed_fee);
}
...
double liquidity_get_rate(name self_contract,
    ...
    liq_info info = {r, p_min, profit_percent, fixed_fee};
    ...
    if (!src_damount) {
        ...
        if (buy) {
            delta_e = value_after_reducing_fixed_fee(info, src_damount);
            ...
            dest_damount = value_after_reducing_fixed_fee(info, delta_e);
        }
