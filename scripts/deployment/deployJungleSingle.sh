...
do
    ...
    if $ISSUE_TOKENS; then
        ...
        #move tokens to reserve owner
        $jleos push action ${ARR_TOKEN_ACCOUNT[$idx]} issue "[ \"$RESERVE_OWNER_ACCOUNT\", \"100000000.${ARR_DECIMALS_ZEROS[$idx]} ${ARR_SYMBOL[$idx]}\", \"memo\" ]" -p ${ARR_TOKEN_ACCOUNT[$idx]}@active
    fi
    ...
    #init 
    $jleos push action ${ARR_RESERVE_ACCOUNT[$idx]} init "[\"$RESERVE_OWNER_ACCOUNT\", \"$NETWORK_ACCOUNT\", \"${ARR_DECIMALS[$idx]},${ARR_SYMBOL[$idx]}\", \"${ARR_TOKEN_ACCOUNT[$idx]}\", \"$EOS_ACCOUNT\", true ]" -p ${ARR_RESERVE_ACCOUNT[$idx]}@active
    ...
    $jleos push action $NETWORK_ACCOUNT addreserve "[ \"${ARR_RESERVE_ACCOUNT[$idx]}\", true ]" -p $NETWORK_OWNER_ACCOUNT@active

    #list reserve pairs                 
    $jleos push action $NETWORK_ACCOUNT listpairres "[ \"${ARR_RESERVE_ACCOUNT[$idx]}\", \"${ARR_DECIMALS[$idx]},${ARR_SYMBOL[$idx]}\", \"${ARR_TOKEN_ACCOUNT[$idx]}\", true ]" -p $NETWORK_OWNER_ACCOUNT@active
...
for idx in 0 1
do
    #move more eos to the new accounts (done by reserev owner)
    $jleos push action $EOS_ACCOUNT transfer "[ \"$RESERVE_OWNER_ACCOUNT\", \"${ARR_RESERVE_ACCOUNT[$idx]}\", \"19.0000 EOS\", \"deposit\" ]" -p $RESERVE_OWNER_ACCOUNT@active
        
    #move tokens to the new account (done by reserev owner)
    $jleos push action ${ARR_TOKEN_ACCOUNT[$idx]} transfer "[ \"$RESERVE_OWNER_ACCOUNT\", \"${ARR_RESERVE_ACCOUNT[$idx]}\", \"100000000.${ARR_DECIMALS_ZEROS[$idx]} ${ARR_SYMBOL[$idx]}\", \"deposit\" ]" -p $RESERVE_OWNER_ACCOUNT@active
done

#set params (done by reserve owner)
$jleos push action ${ARR_RESERVE_ACCOUNT[0]} setparams '[ "0.035", "0.00022"  , "20.0000 EOS", "20.0000 EOS", "0.25", "0.00004",  "0.00001" ]' -p $RESERVE_OWNER_ACCOUNT@active
$jleos push action ${ARR_RESERVE_ACCOUNT[1]} setparams '[ "0.035", "0.00033"  , "20.0000 EOS", "20.0000 EOS", "0.25", "0.00004",  "0.00001" ]' -p $RESERVE_OWNER_ACCOUNT@active    
