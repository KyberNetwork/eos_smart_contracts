...
# deploy network 
####$meos system buyram $ACCOUNT_NAME $NETWORK_ACCOUNT --kbytes 650
####$meos set contract $NETWORK_ACCOUNT contracts/Network Network.wasm --abi Network.abi -p $NETWORK_ACCOUNT@active
####$meos set account permission $NETWORK_ACCOUNT active "{\"threshold\":1, \"keys\":[], \"accounts\":[{\"permission\":{\"actor\":\"$ACCOUNT_NAME\", \"permission\":\"active\"},\"weight\":1}, {\"permission\":{\"actor\":\"$NETWORK_ACCOUNT\", \"permission\":\"eosio.code\"},\"weight\":1}], \"waits\":[] }" owner -p $NETWORK_ACCOUNT@activeexit/

#$meos set account permission $NETWORK_ACCOUNT active "{\"threshold\":1, \"keys\":[], \"accounts\":[{\"permission\":{\"actor\":\"$NETWORK_ADMIN_MSIG\", \"permission\":\"active\"},\"weight\":1}, {\"permission\":{\"actor\":\"$NETWORK_ACCOUNT\", \"permission\":\"eosio.code\"},\"weight\":1}], \"waits\":[] }" owner -p $NETWORK_ACCOUNT@active
#$meos set account permission $NETWORK_ACCOUNT owner "{\"threshold\":1, \"keys\":[], \"accounts\":[{\"permission\":{\"actor\":\"$NETWORK_ADMIN_MSIG\", \"permission\":\"owner\"},\"weight\":1}], \"waits\":[] }" -p $NETWORK_ACCOUNT@owner


####$meos push action $NETWORK_ACCOUNT init "[ \"$ACCOUNT_NAME\", \"$EOS_ACCOUNT\", \"\", false ]" -p $NETWORK_ACCOUNT@active
...
do
    ...
    $meos set account permission ${ARR_RESERVE_ACCOUNT[$idx]} active "{\"threshold\":1, \"keys\":[], \"accounts\":[{\"permission\":{\"actor\":\"${ARR_RESERVE_ADMIN[$idx]}\", \"permission\":\"active\"},\"weight\":1}, {\"permission\":{\"actor\":\"${ARR_RESERVE_ACCOUNT[$idx]}\", \"permission\":\"eosio.code\"},\"weight\":1}], \"waits\":[] }" owner -p ${ARR_RESERVE_ACCOUNT[$idx]}@active
    $meos set account permission ${ARR_RESERVE_ACCOUNT[$idx]} owner "{\"threshold\":1, \"keys\":[], \"accounts\":[{\"permission\":{\"actor\":\"${ARR_RESERVE_ADMIN[$idx]}\", \"permission\":\"owner\"},\"weight\":1}   ], \"waits\":[] }" "" -p ${ARR_RESERVE_ACCOUNT[$idx]}@owner
	...
	$meos push action ${ARR_RESERVE_ACCOUNT[$idx]} setadmin "[\"${ARR_RESERVE_ADMIN[$idx]}\"]" -p $ACCOUNT_NAME@active
done
exit
# deploy network 
####$meos set contract $NETWORK_ACCOUNT contracts/Network Network.wasm --abi Network.abi -p $NETWORK_ACCOUNT@active
